...
{
    public partial class UC_QuanLyThoiGianThi : UserControl
    {

        private ThietLapThoiGianThiDao _thietLapThoiGianThiDao;
        private List<ThoiGianThiModel> studentScoreModels = new List<ThoiGianThiModel>();
        private int _id = 0;
        public UC_QuanLyThoiGianThi(int Height, int Width)
        {
            this.Height = Height;
            this.Width = Width;
            InitializeComponent();
        ...
        public void LoadDataTimeExams(string ten_monhoc = null)
        {
            ...
            studentScoreModels = _thietLapThoiGianThiDao.LayTatCaThoiGianThiCuaMonHoc(ten_monhoc);
            if (studentScoreModels != null && studentScoreModels.Count > 0)
            {
                try
                {
                    foreach (var item in studentScoreModels)
                    {
                        ...
                        newRow.Cells[5].Value = item.ngay_thi;
        ...
        private void btn_delete_Click(object sender, EventArgs e)
        {
            if (_id > 0)
            {
                int result = _thietLapThoiGianThiDao.XoaThoiGianChamThi(_id);
                if (result == 1)
                {
        ...
        private void btn_edit_Click(object sender, EventArgs e)
        {
            if (_id > 0)
            {
                ...
                DateTime ngay_thi = Convert.ToDateTime(row.Cells[5].Value.ToString());
                DateTime ngay_ketthuc = Convert.ToDateTime(row.Cells[6].Value.ToString());
                using (Form_SuaThoiGianThi wd = new Form_SuaThoiGianThi(_id, id_hocky, id_nghanh, ma_monhoc, ten_monhoc, ngay_thi, ngay_ketthuc))
                {
                    wd.ShowDialog();
                }
            }
        ...
        private void btn_report_Click(object sender, EventArgs e)
        {
            ...
            //Save the document
            document.Save("QuanLyThoiGianThi.pdf");
            //Close the document
            document.Close(true);
            System.Diagnostics.Process.Start("QuanLyThoiGianThi.pdf");
        }
